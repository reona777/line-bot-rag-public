...
Google Apps Script（doPost）
       ...
       │    webhookEventIdがない場合はスキップ（fail-safe）
       │    キャッシュ書き込みは回答生成成功後に実行
       │    （GASタイムアウト時にLINEリトライが正常処理されるよう設計）
       ...
       │      最高スコアが閾値（0.6）未満の場合はGemini呼び出しをスキップ
       │      → 全く関係ないチャンクしかヒットしない質問に対し
       │         Geminiが無理に回答を生成することを防ぐ
       │
...
**実測値：** 知識ベース1023チャンク中707件（69%）が内部資料として除外され、  
316件のみがLLMのコンテキストに渡る対象となる。
...
```
loadEmbeddings_: total=1023 public=316 excluded=707
チャンク数: 316
Top1 (sim=0.575): 【特訓コース:高校生・既卒生:〇〇コース】...
回答: 校舎にお問い合わせください。
```

- 監査チャンクは除外されているため、Top1は無関係なコース情報（sim=0.575）
- sim=0.575 < 閾値0.6 のためGemini呼び出し自体がスキップされフォールバック
- **仮にaudienceフィルタだけなら：** Geminiが無関係な文脈で不自然な回答を生成するリスク
- **仮にプロンプト指示だけなら：** 監査チャンクがコンテキストに入り情報漏洩のリスク
- **閾値チェックとの3段構成により：** 上記2つのリスクをどちらも排除
...
LINEはWebhook配信失敗時（HTTP 200以外・タイムアウト）に自動リトライを行う。  
単純に「リクエスト受信時にキャッシュ書き込み → 重複チェック」とすると、  
GASが30秒タイムアウトした際に正当なリトライまでブロックしてしまう。

本実装では以下の2点で対処している：

```javascript
// webhookEventId がない場合は重複チェック自体をスキップ（fail-safe）
var evtKey = event.webhookEventId ? 'rag_evt_' + event.webhookEventId : null;
if (evtKey && cache.get(evtKey)) return;

// ... RAG処理 ...

// キャッシュ書き込みは回答生成成功後（タイムアウト時は書き込まれずリトライが通る）
if (evtKey) cache.put(evtKey, '1', 60);
```

### Drive読み込みのスケール限界

`loadEmbeddings_()` はリクエストごとにGoogle DriveからJSONを読み込む。  
CacheServiceの上限（100KB）に対してベクトルデータ（~数十MB）は大きすぎるためキャッシュ化できない。  
同時アクセスが増えるとDrive APIクォータに引っかかる可能性があるため、  
ユーザー規模が数百人を超えたらCloud FunctionsやCloud Runへの移行を検討する。
